...
class Agent(BaseAgent):
  ...
  def learn(self, trajs):
    ...
    # Action logic
    with torch.no_grad():
      _, logprobs, _, values = self.model.get_action_and_value(obs.view(-1, args.obs_dim), actions.long().view(-1))
    print(f"{logprobs.shape=}, {values.shape=}")
    values = values.view(num_steps, num_sample)
    logprobs = logprobs.view(num_steps, num_sample)
    ...
    if now - self.last_opt_time > 10:
      clean_ckpt_memory(logger=self.logger)
